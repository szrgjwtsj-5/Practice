...
import com.whx.practice.view.leaf.UiUtils.dp2px
...
class RoundImageView @JvmOverloads constructor(
    ...
    defArr: Int = 0
) : AppCompatImageView(context, attrs, defArr) {

    private var isCircle = false // 是否显示为圆形，如果为圆形则设置的corner无效
    private var isCoverSrc = false// border、inner_border是否覆盖图片
    private var borderWidth = 0 // 边框宽度
    private var borderColor: Int = Color.WHITE // 边框颜色

    private var innerBorderWidth = 0 // 内层边框宽度
    ...
    private var srcRadii = FloatArray(8)

    private var srcRectF = RectF() // 图片占的矩形区域
    private var borderRectF = RectF() // 边框的矩形区域

    private var paint = Paint()
    private var path = Path() // 用来裁剪图片的ptah

    private lateinit var srcPath: Path // 图片区域大小的path
    ...
    private fun initAttrs(attrs: AttributeSet?) {
        val ta: TypedArray = context.obtainStyledAttributes(attrs, R.styleable.RoundImageView, 0, 0)
        for (i in 0 until ta.indexCount) {
            when (val attr = ta.getIndex(i)) {
                R.styleable.RoundImageView_is_cover_src -> {
                    isCoverSrc = ta.getBoolean(attr, isCoverSrc)
                }
                R.styleable.RoundImageView_is_circle -> {
                ...
                R.styleable.RoundImageView_inner_border_width -> {
                    innerBorderWidth = ta.getDimensionPixelSize(attr, innerBorderWidth)
                }
                R.styleable.RoundImageView_inner_border_color -> {
    ...
    override fun onDraw(canvas: Canvas) { // 使用图形混合模式来显示指定区域的图片
        canvas.saveLayer(srcRectF, null, ALL_SAVE_FLAG)
        if (!isCoverSrc) {
            val sx =
